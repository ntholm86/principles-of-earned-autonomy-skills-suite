...
Do the work. **For decision-bearing runs** (anything tiering Standard or Full in step 3), perform Improve-grade reasoning: examine the target, challenge your first reading, then decide. **For Micro runs** (genuinely mechanical asks with no real choice), skip to action.

**Examination** — build a model of the target and what matters. Four lenses are available as thinking tools, not a checklist: **Purpose** (does the target achieve what it's for, against `.acm/destination.md`?), **Inconsistency** (does it contradict itself or its own stated rules?), **Overburden** (does it ask more of its users or maintainers than it's worth?), **Waste** (does everything here still earn its place?) — plus whatever the target and domain require (security, performance, correctness, human legibility...). Derive target-specific lenses when the named four don't fit; name only the ones that were actually load-bearing. When the examination is a fidelity audit — checking a derivative against its source (a consolidation against its suite, a summary against its document, a port against its original) — compare against the source directly; self-review of the derivative alone reliably misses what was dropped.

**Map before reasoning** is the default. Establish a compact view of the target's major parts, controlling relationships, and important unknowns before acting locally. How broad? Agent judgment, scaled by domain risk and reversibility. No universal read count; make the chosen breadth inspectable in Trail.
...
**Challenge** your reading before deciding. Ask: *What am I not seeing? Am I anchored on the most obvious finding and missing a subtler, more important one?* Consider competing routes or interpretations; test whether one dominates. Then the redesign question: *is the target's structure itself wrong, so that no incremental fix will help?* If the structure itself is wrong, don't patch — argue for redesign: sketch the alternative, weigh redesign cost against the cost of continuing to patch, and stop for the operator's decision. This challenge operation is mandatory for decision-bearing work, abbreviated in Standard recording, detailed in Full.
...
- **Tier 2 — Standard** (a real choice existed between plausible alternatives, or the work could plausibly be second-guessed later). **Reasoning operations:** full Improve-grade (examination, challenge, decision). **Recording depth:** compact. Write interpretation (only if it wasn't obvious), examination in 1-2 sentences naming load-bearing lenses, challenge in one line if a competing route was considered, decision plus a one-line prediction, outcome vs. prediction, one line naming the blind spot (or the nothing-found), and one line naming the most obvious next-step candidate if one stands out — skip it if none does; silence is valid here too. Skip the full reflection ceremony unless one of the escalation triggers below fires.

  ```
  ## 2026-07-03 — split-config-loader
  Read "clean up config handling" as splitting the loader, not rewriting it.
  Examination: Purpose lens — the config loader does 3 jobs (parse, validate, env-merge); splitting the validation clears one inconsistency (error messages that name the wrong source file).
  Challenge: rewrite-from-scratch was plausible but riskier; split preserves tests and deployed behavior.
  [!DECISION] Predicted the split leaves all 12 tests green; it did.
  Blind spot: no test covers the env-override path. Next: cover it.
  Cost: moderate — 9 tool ops, 4 files, no subagent.
  ```

- **Tier 3 — Full** (structural or architectural decisions, redesign arguments, anything the operator would want deeply audited, anything touching an area a past `[!REVERSAL]` or a recurring finding-class — the same kind of defect turning up across separate runs — already flagged). **Reasoning operations:** identical to Standard (examination, challenge, decision). **Recording depth:** complete. The full prose shape: interpretation, examination with lenses named and findings detailed, challenge with competing routes weighed, decision + prediction, action, reflection (a falsifiable claim about the target, a named blind spot, an imagined expert's pushback), the four across-trail triggers each explicitly evaluated — *did the operator ask for a deeper audit? is a recurring finding-class present? does this run contradict a prior `[!REALIZATION]`? is a silence declaration imminent?* — and candidate next moves.
