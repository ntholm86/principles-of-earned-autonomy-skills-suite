...
- **The `harness/tools/` tier is brought up to the current contracts.** Three migrations had reached every contract and doc but not the optional tooling: `harness/tools/hooks/pre-commit` carried genuine mojibake in its docstring since 2026-06-21 (invisible because `verify.py check_no_mojibake` skipped suffixless files), still pointed installers at `tools/install-hooks.*` (moved to `harness/tools/` 2026-05-23), and its `SUBSTANTIVE` regex still matched `tools/.*\.py`, so `harness/tools/*.py` changes had not counted as substantive since the move; `record.py`'s `new` stub still prompted `agent: (provider, tool-call ID prefix)` - the exact family-unassignable form Trail 2.5.5 outlawed in v4.33.11 - plus a `session-file: .acm/sessions/...` line for the agent-authored summaries retired 2026-08-01, and two section headings that diverged from Trail's template. The hook docstring is ASCII and names `<skills-repo>/harness/tools/install-hooks.*`; `SUBSTANTIVE` matches `(?:harness/)?tools/.*\.py`; the stub's `agent:` hint states model family and version, the `session-file` line is gone, headings match `### Action` / `### Candidate Next Moves`, and `record.py new` reports the scope-resolved path; `check_no_mojibake` now scans suffixless files (confirmed: it fails on the pre-fix hook, passes after). No contract text changed. Found by a fresh-session Claude Fable 5.1 convergence evaluation of the suite at 87b4b33.

## v4.34.0 - 2026-09-09

- **Named task scopes inside a repo's `.acm/`.** One repo carrying several unrelated tasks produced one trail past 20k lines that mixed arcs sharing nothing but a directory (operator field report from professional use). ACM §4 discovers scopes as `.acm/` directories up the filesystem, which needs a directory per scope; tasks that share a directory now get a named scope inside the repo's `.acm/` (`.acm/<task>/audit-trail.md`, optional `orientation.md`, `destination.md`, derived files). The hierarchy is strict and is ACM §4.3 one level down: workspace governs repo, repo governs task, the higher-level `.acm/` always wins. Trail defines the structure and writes only the active scope; Intent selects the scope as part of interpretation and names it in the narration (existing task, repo, or a new task named by the prompt - never a silent side effect); Improve, Orient, and Destination read parent scopes first and read/write the active scope. `record.py` gains `--scope <name>` / `$ACM_SCOPE` with unchanged default behavior; the pre-commit hook accepts a task-scope trail. Flagged as a candidate addition to the ACM spec (§4) upstream. `trail/SKILL.md` 2.5.5 -> 2.6.0; `intent/SKILL.md` 1.8.2 -> 1.9.0; `improve/SKILL.md` 3.19.4 -> 3.20.0; `orient/SKILL.md` 2.7.2 -> 2.8.0; `destination/SKILL.md` 2.8.3 -> 2.9.0.
...
- **Silence records now carry what Principle 3 needs to count them.** Trail's `agent:` field was `<model / provider>` with no definition; Improve's silence bound named bar, scope, and untested bars but not who evaluated, which artifact, or whether the session was fresh. Principle 3 counts same-family evaluators as one and requires fresh sessions on an unchanged artifact, and the Destination's convergence lease (2026-08-09) depends on that rule — yet 98 of 310 trail entries (32%, 2026-05-05 to 2026-08-19) record only `GitHub Copilot`, three bounded-silence verdicts among them are family-unassignable, and a 2026-09-05 entry recorded "identity not independently verified" despite the operator naming the model. Trail now defines `agent:` as model family and version (operator-stated identity marked as such; the host product is not a family) and has silence commits carry verdict and evaluator in the subject; Improve 4a's silence bound adds artifact identity, evaluator family/version, and session independence; Improve step 7's fallback metadata line matches. verify.py and historical entries are unaffected. Found by a fresh-session Claude Fable 5.1 convergence evaluation of the suite at c4e4757. `trail/SKILL.md` 2.5.4 -> 2.5.5; `improve/SKILL.md` 3.19.3 -> 3.19.4.
