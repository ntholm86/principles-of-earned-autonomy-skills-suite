...
1. `.acm/destination.md` — the **operator-held destination**. What the target is for, the constraints that hold across all runs, the hard problem the work is trying to chip away at. Stable: revised only by the operator, or by Destination after the operator has confirmed the synthesized meaning; no other skill writes it. If present, it is the highest-priority context — it tells you what success means before you decide what to change.
